...
void GuiMenu::menuControllers() {
	...
	row.addElement(
			std::make_shared<TextComponent>(window, _("CONFIGURE A CONTROLLER"), mMenuTheme->menuText.font, mMenuTheme->menuText.color),
			true);
	...
	row.addElement(
			std::make_shared<TextComponent>(window, _("PAIR A BLUETOOTH CONTROLLER"), mMenuTheme->menuText.font, mMenuTheme->menuText.color),
			true);
	...
	row.addElement(
			std::make_shared<TextComponent>(window, _("FORGET BLUETOOTH CONTROLLERS"), mMenuTheme->menuText.font, mMenuTheme->menuText.color),
			true);
...
void GuiMenu::menuNetworkSettings(){
	...
	auto WifiSSID = [this, baseSSID, enable_wifi, baseEnabled] (GuiSettings *gui, std::string title,
		...
		auto lbl = std::make_shared<TextComponent>(mWindow, title, mMenuTheme->menuText.font,
												   mMenuTheme->menuText.color);
		row.addElement(lbl, true); // label

		std::shared_ptr<GuiComponent> ed;

		ed = std::make_shared<TextComponent>(mWindow, value, mMenuTheme->menuText.font,
											 mMenuTheme->menuText.color, ALIGN_RIGHT);
		row.addElement(ed, true);
		...
		row.makeAcceptInputHandler([this, updateVal, enable_wifi, baseEnabled] {
			...
			ed = std::make_shared<TextComponent>(mWindow, _("MANUAL INPUT"),
												 mMenuTheme->menuText.font,
												 mMenuTheme->menuText.color, ALIGN_LEFT);
			row.addElement(ed, true);
...
void GuiMenu::menuQuit(){
	...
	row.addElement(
			std::make_shared<TextComponent>(window, _("RESTART SYSTEM"), mMenuTheme->menuText.font, mMenuTheme->menuText.color), true);
	s->addRow(row);
	...
	row.addElement(
			std::make_shared<TextComponent>(window, _("SHUTDOWN SYSTEM"), mMenuTheme->menuText.font, mMenuTheme->menuText.color), true);
	s->addRow(row);
	...
	row.addElement(
			std::make_shared<TextComponent>(window, _("FAST SHUTDOWN SYSTEM"), mMenuTheme->menuText.font, mMenuTheme->menuText.color), true);
	s->addRow(row);
	mWindow->pushGui(s);
}
